...
#define LWMainThreadAssert() NSAssert([NSThread isMainThread], @"LWHUD needs to be accessed on the main thread.");

CGFloat const LWProgressMaxOffset = 1000000.f;

static const CGFloat LWDefaultPadding = 4.f;
static const CGFloat LWDefaultLabelFontSize = 16.f;
static const CGFloat LWDefaultDetailsLabelFontSize = 12.f;
...
@interface LWHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    LWHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    LWHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (LWHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            LWHUD *hud = (LWHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = LWHUDAnimationFade;
    _mode = LWHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(LWHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == LWHUDAnimationZoom) {
        type = animatingIn ? LWHUDAnimationZoomIn : LWHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == LWHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == LWHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == LWHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == LWHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    LWHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<LWHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    LWBackgroundView *backgroundView = [[LWBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = LWHUDBackgroundStyleSolidColor;
    ...
    LWBackgroundView *bezelView = [LWBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:LWDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:LWDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [LWHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:LWDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[LWRoundProgressView class]];

    LWHUDMode mode = self.mode;
    if (mode == LWHUDModeIndeterminate) {
    ...
    else if (mode == LWHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[LWBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LWHUDModeDeterminate || mode == LWHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[LWRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == LWHUDModeAnnularDeterminate) {
            [(LWRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == LWHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LWHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[LWHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[LWHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[LWRoundProgressView class]]) {
        LWRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [LWRoundProgressView appearanceWhenContainedIn:[LWHUD class], nil];
#else
        appearance = [LWRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[LWHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((LWRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((LWRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[LWBarProgressView class]]) {
        LWBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [LWBarProgressView appearanceWhenContainedIn:[LWHUD class], nil];
#else
        appearance = [LWBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[LWHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((LWBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((LWBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    LWBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? LWDefaultPadding : 0.f;
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == LWHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation LWRoundProgressView
...
@implementation LWBarProgressView
...
@implementation LWBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = LWHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = LWHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(LWHUDBackgroundStyle)style {
    if (style == LWHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = LWHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    LWHUDBackgroundStyle style = self.style;
    if (style == LWHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == LWHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation LWHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [LWHUD allHUDsForView:view];
    for (LWHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable LWHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    LWBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == LWHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    LWMainThreadAssert();
    self.backgroundView.style = LWHUDBackgroundStyleSolidColor;
...
@implementation LWHUDRoundedButton
